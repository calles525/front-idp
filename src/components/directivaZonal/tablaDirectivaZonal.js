...
const TablaDirectivaZonal = () => {
    ...
    const fetchData = async () => {
        setActivate(true)
        try {
            const response = await fetch(op.conexion+"/api/directivazonal/alldirectivos?zona="+idzona);
            if (response.ok) {
    ...
    const allCargos = async () => {
        setActivate(true)
        try {
            const response = await fetch(op.conexion+"/api/directivazonal/allcargos?sociedad=4&nivel='Zonal'");
            if (response.ok) {
                const result = await response.json();
    ...
    const fetcAllIglesias = async () => {
        setActivate(true)
        try {
            const response = await fetch(op.conexion+"/api/iglesia/alliglesias?zona=" + idzona);
            if (response.ok) {
                const result = await response.json();
                console.log("Datos iglesias:", result); // Asegúrate de que los datos sean correctos
                setIglesias(result);
            } else {
                console.error("Error al obtener los datos:", response.statusText);
            }
    ...
    const handleSubmit = async () => {
        ...
        // Incluir
        try {
            const response = await fetch(op.conexion+"/api/directivazonal/registrardirectivo", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    id_directiva_zonal: formData.id_directiva_zonal,
                    p_email:formData.correo,
                    i_idiglesia:formData.idiglesia
    ...
    return (
        <div>
            ...
            <Card>
                <CardBody>
                    <CardTitle tag="h5">Directiva JC {nombrezona} </CardTitle>
...
{parseInt(operacionPer) === 1 ? <div class="input-group input-group-sm mb-3 mt-3">
                            <span class="input-group-text" id="inputGroup-sizing-sm">Iglesia:</span>
                            <select disabled={operacion === 4} id="id_iglesia" name="id_iglesia" onChange={handleInputChange} value={formData.id_iglesia} className="text-uppercase form-control">
                                <option value="" disabled selected>
                                    -- Seleccione una opción --
                                </option>
                                {iglesias.map((item) => (
                                    <option key={item.id_iglesia} value={item.id_iglesia}>
                                        {item.nombrei}
                                    </option>
                                ))}
                            </select>
...
export default TablaDirectivaZonal;
